{{-- File: resources/views/admin/systems/edit.blade.php (Using App Layout & Tailwind) --}}
<x-app-layout>
    <x-slot name="header">
        {{ __('ویرایش سامانه:') }} {{ $system->name ?? $system->id }}
    </x-slot>

    <div class="p-6 bg-white rounded-lg shadow border">
        {{-- Ensure route 'admin.systems.update' exists --}}
        <form action="{{ route('admin.systems.update', $system) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    {{-- Ensure $customers is passed from the controller's edit method --}}
                    <select name="customer_id" id="customer_id" class="mt-1 block w-full px-3 py-2 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm @error('customer_id') border-red-500 @enderror" required>
                        ...
                        @isset($customers)
                            @foreach($customers as $customer)
                                <option value="{{ $customer->id }}" {{ old('customer_id', $system->customer_id) == $customer->id ? 'selected' : '' }}>
                                    {{ $customer->name }}
                                </option>
                            @endforeach
                        @else
                ...
                <div>
                    ...
                    <input type="url" name="domain" id="domain" value="{{ old('domain', $system->domain) }}" required placeholder="https://example.com"
                ...
                <div>
                    ...
                    <select name="status" id="status" class="mt-1 block w-full px-3 py-2 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm @error('status') border-red-500 @enderror" required>
                        <option value="active" {{ old('status', $system->status) == 'active' ? 'selected' : '' }}>فعال</option>
                        <option value="inactive" {{ old('status', $system->status) == 'inactive' ? 'selected' : '' }}>غیرفعال</option>
                    </select>
                ...
                {{-- Current Version --}}
                 <div class="md:col-span-2">
                    ...
                    <input type="text" name="current_version" id="current_version" value="{{ old('current_version', $system->current_version) }}"
